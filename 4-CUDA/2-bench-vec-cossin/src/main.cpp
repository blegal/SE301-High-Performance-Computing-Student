...
#include "vcossin/vec_cossin_x86.hpp"
#include "cuda/CUDACosSin.hpp"
#include <cstring>
#include <chrono>
#include <random>
...
bool is_ok(const float* ref, const float* eva, const int length)
{
    ...
    {
        if( std::fabs(eva[i] - ref[i]) > 1e-5 ) // On regarde l'équivalence !!!
        {
            printf("i = %4d | eva[%f] != ref[%f] => epsilon = %f\n", i, eva[i], ref[i], std::fabs(eva[i] - ref[i]));
            ok = false;
        }
    }
    return ok;
...
int main(int argc, char* argv[])
{
    ...
    for(int buffSize = v_begin; buffSize <= v_end; buffSize *= v_step)
    {
        ...
        for(int32_t loop = 0; loop < nTest; loop += 1)
            vec_cossin_x86_novect(o_nov, a_tab, b_tab, buffSize);
        ...
        for(int32_t loop = 0; loop < nTest; loop += 1)
            vec_cossin_x86(o_x86, a_tab, b_tab, buffSize);
        ...
        for (int threads = 1; threads <= max_threads; threads++)
        {
            ...
            for(int loop = 0; loop < nTest; loop += 1)
                vec_cossin_omp(o_omp, a_tab, b_tab, buffSize);
        ...
        CUDACosSin c_cossin( buffSize );
        c_cossin.load_A(a_tab);
        c_cossin.load_B(b_tab);
        auto start_i_mtl = std::chrono::system_clock::now();
        for(int32_t loop = 0; loop < nTest; loop += 1)
            c_cossin.execute();
        c_cossin.store_C(o_mtl);
