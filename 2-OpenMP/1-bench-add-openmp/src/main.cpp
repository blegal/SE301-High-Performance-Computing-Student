...
#else
    printf("(II) Stage 1 - The vector sum example like previously on Windows\n");
#endif
...
int main(int argc, char* argv[])
{
    ...
    for(int buffSize = v_begin; buffSize <= v_end; buffSize *= v_step)
    {
        ...
        printf("    - [-x86-] unoptimized     \033[32mOK\033[0m [%6d ns]\n", (int32_t)time_ref);
        ...
        printf("    - [AUTOV] auto-vectorized \033[32mOK\033[0m [%6d ns]\n", (int32_t)time_x86);
        ...
        for (int threads = 1; threads <= max_threads; threads++)
        {
            ...
            if( is_ok(o_x86, o_omp, buffSize) ){
                printf("    - [OMP%2d] multi-core mode \033[32mOK\033[0m [%6d ns]\n", threads, (int32_t)time_omp);
            }else{
                printf("    - [OMP%2d] multi-core mode \033[31mKO\033[0m [%6d ns]\n", threads, (int32_t)time_omp);
                exit( EXIT_FAILURE );
            }
        }
