...
#include "vmax_index/vec_max_index_threads.hpp"
#include <omp.h>
#include <cstring>
#include <chrono>
#include <thread>
...
void header( )
{
    ...
    printf("(II) Stage 5 - OpenMP is fun but let's make it ourself on MacOS\n");
#elif defined (__linux__)    
    printf("(II) Stage 5 - OpenMP is fun but let's make it ourself on Linux\n");
#else
    printf("(II) Stage 5 - OpenMP is fun but let's make it ourself on Windows\n");
#endif
...
//                continue;
            ...
            for(int loop = 0; loop < nTest; loop += 1)
                o_omp = vec_max_index_threads(a_tab, buffSize, threads);
