...
#ifdef __ARM_NEON__
inline void uint8_t_show(const uint8x8_t reg)
{
    uint8_t tmp[sizeof(uint8x8x2_t)];
//    vst4_s8(pDst, src_reg1);
    vst1_u8 (tmp, reg);
    for(int i=0; i < sizeof(uint8x8_t); i++)
        printf("%3d ", tmp[sizeof(uint8x8_t)-1-i]);
    printf("\n");
}
#endif
...
inline void uint8_t_show(const __m256i reg)
{
    ...
    for(int i=0; i<(int)sizeof(__m256i); i++)
...
inline void int8_t_show(const int8_t* ptr, const int length)
{
    assert( ptr    != nullptr );
    assert( length !=       0 );
...
inline void uint8_t_show(const uint8_t* ptr, const int length)
{
    assert( ptr    != nullptr );
    assert( length !=       0 );
    for(int i = 0; i < length; i+= 32)
